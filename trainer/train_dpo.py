...
def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    # ref_log_probs and policy_log_probs both have shape: (batch_size, seq_len)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save_weight', default='dpo', type=str, help="Prefix name for saved weights")
    parser.add_argument("--epochs", type=int, default=1, help="Number of training epochs")
    parser.add_argument("--batch_size", type=int, default=4, help="batch size")
    parser.add_argument("--learning_rate", type=float, default=4e-8, help="Initial learning rate (recommended <=5e-8 to avoid forgetting)")
    ...
    parser.add_argument('--max_seq_len', default=1024, type=int, help="Maximum truncation length for training (Chinese: 1 token ≈ 1.5~1.7 characters)")
    ...
    parser.add_argument('--from_weight', default='full_sft', type=str, help="Which weight to train from")
    parser.add_argument('--from_resume', default=0, type=int, choices=[0, 1], help="Whether to auto-detect & resume training (0=No, 1=Yes)")
    parser.add_argument('--beta', default=0.15, type=float, help="Beta parameter in DPO")
    ...
    Logger(f'Policy model total parameters: {sum(p.numel() for p in model.parameters()) / 1e6:.3f} M')
    # Initialize reference model (ref_model is frozen)
    ref_model, _ = init_model(lm_config, args.from_weight, device=args.device)
    ref_model.eval()
    ref_model.requires_grad_(False)
    Logger(f'Reference model total parameters: {sum(p.numel() for p in ref_model.parameters()) / 1e6:.3f} M')
    ...
    # ========== 7. Compile and distributed wrapping ==========
